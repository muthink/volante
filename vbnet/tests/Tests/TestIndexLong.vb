Namespace Volante
	...
	Public Class TestIndexLong
		...
		Const min As Long = Long.MinValue
		Const max As Long = Long.MaxValue
		Const mid As Long = 0

		Private Shared Function Clamp(n As Long) As Long
			Return n
		End Function
		...
		Public Sub Run(config As TestConfig)
			...
			Dim idx = db.CreateIndex(Of Long, Record)(IndexType.NonUnique)
			db.Root = idx
			Dim val As Long = 1999
			For i = 0 To count - 1
				Dim idxVal As Long = Clamp(val)
				r = New Record(idxVal)
				idx.Put(idxVal, r)
				If i Mod 100 = 0 Then
					db.Commit()
