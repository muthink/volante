Namespace Volante
	...
	Public Class TestIndexULong
		...
		Const min As ULong = ULong.MinValue
		Const max As ULong = ULong.MaxValue
		Const mid As ULong = max \ 2

		Private Shared Function Clamp(n As Long) As ULong
			Return CULng(n)
		End Function
		...
		Public Sub Run(config As TestConfig)
			...
			Dim idx = db.CreateIndex(Of ULong, Record)(IndexType.NonUnique)
			db.Root = idx
			Dim val As Long = 1999
			For i = 0 To count - 1
				Dim idxVal As ULong = Clamp(val)
				r = New Record(idxVal)
				idx.Put(idxVal, r)
				If i Mod 100 = 0 Then
					db.Commit()
