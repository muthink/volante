Namespace Volante
	...
	Public Class TestIndexDecimal
		...
		Const min As Decimal = Decimal.MinValue
		Const max As Decimal = Decimal.MaxValue
		Const mid As Decimal = 0

		Private Shared Function Clamp(n As Long) As Decimal
			Return n
		End Function
		...
		Public Sub Run(config As TestConfig)
			...
			Dim idx = db.CreateIndex(Of Decimal, Record)(IndexType.NonUnique)
			db.Root = idx
			Dim val As Long = 1999
			For i = 0 To count - 1
				Dim idxVal As Decimal = Clamp(val)
				r = New Record(idxVal)
				idx.Put(idxVal, r)
				If i Mod 100 = 0 Then
					db.Commit()
