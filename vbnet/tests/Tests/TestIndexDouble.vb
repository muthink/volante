Namespace Volante
	...
	Public Class TestIndexDouble
		...
		Const min As Double = Double.MinValue
		Const max As Double = Double.MaxValue
		Const mid As Double = 0

		Private Shared Function Clamp(n As Long) As Double
			Return n
		End Function
		...
		Public Sub Run(config As TestConfig)
			...
			Dim idx = db.CreateIndex(Of Double, Record)(IndexType.NonUnique)
			db.Root = idx
			Dim val As Long = 1999
			For i = 0 To count - 1
				Dim idxVal As Double = Clamp(val)
				r = New Record(idxVal)
				idx.Put(idxVal, r)
				If i Mod 100 = 0 Then
					db.Commit()
